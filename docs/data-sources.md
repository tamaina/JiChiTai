...
- `public/generated/dictionaries/`: 全国一括・都道府県別IME辞書と形式別ZIP
...
`pnpm ime-dictionaries:build`は生成済み自治体マスターを入力として、macOS plist、Microsoft IME、Google日本語入力/Mozc向けの全国一括辞書と都道府県別辞書を生成します。各自治体について正式名と市・区・町・村を除いた表記を収録し、都道府県は都・府・県を除きます。北海道だけは正式な「北海道」を維持します。Microsoft IMEとGoogle/Mozcのコメント欄には都道府県名と、データがある場合は郡名を記録します。
