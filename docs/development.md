...
自治体マスターや境界生成処理を変更した場合は`pnpm geo:build`を実行します。この処理はe-StatのローカルCSVに加え、行政界と湖沼データをネットワークから取得して全国1,747自治体のSVGを再生成します。生成後は`pnpm exec prettier --write src/shared/data/generated-municipalities.ts`を実行してから`pnpm check`で検証します。
