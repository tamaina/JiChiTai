...
test('searches municipalities and opens the selected detail', async ({
  ...
  await expect(tottori).toContainText('本庁代表電話 0857-22-8111')

  await page.getByLabel('郵便番号').check()
...
test('explores prefectures and municipality details from the map and list', async ({
  ...
  await expect(focusedRegion).toHaveCSS('filter', 'none')
  await expect(focusedRegion).toHaveCSS('fill', /color|rgb/)
...
test('answers a municipality from a licensed municipality emblem', async ({
  page,
}) => {
